...
// Synthetic corporate material with deterministic keywords for the content
// classifier and local rule extraction (no OCR needed).
function corporateMaterial(name: string): { name: string; mimeType: string; buffer: Buffer } {
  return {
    name: '借款申请书.md',
...
/** Adopt one extracted candidate by its field label. */
...
/** Enter one manual confirmed value (no material source) through the dialog. */
...
test('企业演示流程：登录 → 上传 → 候选确认 → 完备性 → 红线 → 辅助审查完成', async ({ page }) => {
  const username = process.env.E2E_USERNAME
  const password = process.env.E2E_PASSWORD
  if (!username || !password) throw new Error('E2E_USERNAME and E2E_PASSWORD are required')

  await login(page, username, password)
  const borrowerName = `企业演示-${Date.now()}`
  await page.getByLabel('主借款人类型').selectOption('corporate')
  ...
  await page.locator('input[type="file"]').setInputFiles(corporateMaterial(borrowerName))
  await expect(page.getByText('处理成功')).toBeVisible({ timeout: 60_000 })

  // candidate review: adopt the extracted proposed-loan candidates
  ...
  // 罚息利率 has no extraction rule; enter it by hand
  await confirmManual(page, '罚息利率', '18%', '演示录入：罚息利率 18%')
  await expect(page.getByText('贷款金额', { exact: true }).first()).toBeVisible()

  // completeness: confirm classification + mapping, run the formal report
  ...
  await page.getByRole('combobox', { name: '选择类别-借款申请书.md' }).focus()
  ...
  await page.getByRole('button', { name: '确认分类-借款申请书.md' }).click()
  await page.getByRole('button', { name: '确认映射-借款申请书.md-借款申请书' }).click()
  ...
  // auxiliary review completion from the application detail page
  await page.getByRole('link', { name: '返回申请列表' }).click()
  await page.getByRole('link', { name: borrowerName }).click()
